...
**LabVIEW VI Support** 是一款用于在 Visual Studio Code 中预览并编辑 LabVIEW VI（`.vi`）/ 模板（`.vit`）文件的扩展。它通过 Windows Script Host + LabVIEW COM/ActiveX 桥接，让你无需切换到 LabVIEW IDE 即可查看静态属性、按需读取动态属性、预览前面板 / 程序框图截图，并直接修改常用 VI 属性。
...
4. 默认先展示无需连接 LabVIEW 即可获得的静态属性（如文件名、路径、保存版本）。
5. 点击 **读取动态属性** 后再加载可写属性详情；当视图模式包含预览区域时，扩展才会按需导出前面板 / 程序框图图像。
6. 相同 VI（相同内容哈希）再次打开时会直接命中缓存，近乎即时响应。
...
- 默认首次打开仅显示属性表；切换到预览模式后才会触发前面板（FP）与程序框图（BD）图像导出。
- 属性读取分为两段：先展示静态属性，再按需读取动态属性。
